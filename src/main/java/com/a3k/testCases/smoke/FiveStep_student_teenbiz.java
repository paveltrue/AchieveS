...
public class FiveStep_student_teenbiz extends BasicTestCase {
	...
	public void verifyLesson_TeenBizStudent(
		...
		softAssert.assertTrue(lessonFound,
	...
	protected void verifyArticlePage(Lesson lesson) {
		...
		softAssert.assertTrue(lesson.verifyPresenceWordsLinks(), "There is no words links on Article page");
		softAssert.assertTrue(lesson.verifyPresenceImage(), "There is no image on Article page");
		softAssert.assertTrue(lesson.verifyPresenceCredentials(), "There is no author credentials on Article page");
		softAssert.assertTrue(lesson.verifyPresencePrintButton(), "There is no print button on Article page");
	...
	protected void verifyAfterReadingPollTab(Lesson lesson) {
		...
		softAssert.assertTrue(lesson.verifyPresenceQuestion(), "The question is not appears on After Reading Poll Tab");
		softAssert.assertTrue(lesson.verifyPresenceRadiobuttons(), "Radio buttons are not displayed on After Reading" + " Poll Tab");
		softAssert.assertTrue(lesson.verifyPresenceSubmitButtonART(), "The Submit button is not displayed on After Reading Poll Tab");

		softAssert.assertTrue(lesson.checkRadiobuttonsClickable(), "Option is not selected");
	}
	@Step
	protected void verifyActivityPage(Lesson lesson, String testName) throws InterruptedException {
		...
		softAssert.assertEquals(answers, 4, "Must be 4 answers choice, but found " + answers);
		...
		softAssert.assertTrue(highlighting, "The selected answer is not highlighted");
		lesson.clickActivitySubmitButton();

		if (lesson.isAnswerWasInccorect()) {
			softAssert.assertTrue(lesson.verifyVisibleMotivationMessage(), "Motivation Message is not appeared on Activity Tab");
			...
			softAssert.assertTrue(lesson.verifyVisibleNextQuestionButton(), "There is no Next Question button on Activity Tab");

			lesson.clickNextQuestionButton();
			int currentPoins = lesson.currentPoints();
			String newQuestion = lesson.currentQuestion();

			softAssert.assertNotEquals(newQuestion, questionNumber, "New question is not opened");

			if (!testName.equals("spark")) {
				softAssert.assertTrue(currentPoins > pointsBeforeAnswer, "Points are not incremented");
			}
		}
	...
	protected void verifyFinishLaterButton(Lesson lesson) {
		...
		softAssert.assertEquals(popupContent, finishLaterPopupContent, "Expected popup text does not equal to actual." + " Actual: " + popupContent + ". Expected: " + finishLaterPopupContent);
	}
	@Step
	protected void verifySubmitButton(Lesson lesson) {
		...
		softAssert.assertEquals(popupContent, submitPopupContainer, "Actual popupContent does not equal to expected." + " Actual: " + popupContent + ". Expected: " + submitPopupContainer);
	}
	@Step
	protected void verifyMathTab(Lesson lesson) {
		...
		softAssert.assertTrue(lesson.verifyHighlightActivityWordMath(), "'Activity' word is not highlighted");

		softAssert.assertTrue(lesson.verifyDisableResultsWordMath(), "'Results' word is not disabled");

		softAssert.assertTrue(lesson.verifyPrecenceAcitivityQuestion(), "Text with question is not displayed");

		softAssert.assertTrue(lesson.verifyPresenceActivitySubmitButtonMath(), "Submit button is not present");

		int answers = lesson.getPresenceFourAnswerOptionsForMath();
		softAssert.assertTrue(totalQuestions == 1, "There is no questions on Math Tab");
	...
	protected void tryAgain(Lesson lesson) {
		...
		while (lesson.isTryAgainButtonVisible()) {
			...
			softAssert.assertTrue(lesson.verifyVisibleMotivationMessageMath(), "Motivation Message is not appeared");
	...
	protected void verifyStretchTab(Lesson lesson, String testName) {
		...
		if (!lesson.isStretchTabBlocked()) {
			...
			softAssert.assertTrue(lesson.verifyPresenceResultsTable(), "Result table is not present");
		}
	}
	@Step
	protected void verifyReadingConnections(Lesson lesson) {
		...
		softAssert.assertTrue(lesson.verifyPresenceReadingConnectionsMarks(), "There is no reading connections on Article page");
	...
	public Object[][] getUsers(ITestContext context) {
		...
		case "SmokeTestUK":
			data = new Object[][] { { "teenbizstud.uk", "teenbizstud.uk", "", "", "english", "smoke" }, };
			break;

		case "SpanishTest":
			data = new Object[][] { { "autoteenbizsp.stud", "autoteenbizsp.stud", "", "Second Teenbiz Class", "spanish", "spanish" }, };
			break;

		case "SparkTest":
			data = new Object[][] { { "sparkstudent.one", "sparkstudent.one", "", "Spark First Class", "english", "spark" }, };
			break;
